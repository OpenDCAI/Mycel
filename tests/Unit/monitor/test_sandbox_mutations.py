...
def test_cleanup_sandbox_prunes_workspace_rows_after_runtime_destroy(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *, sandbox_runtime_handle, provider_name, detach_thread_bindings: (
            destroyed.append((sandbox_runtime_handle, provider_name, detach_thread_bindings))
            or {"ok": True, "action": "destroy", "provider": provider_name, "sandbox_runtime_handle": sandbox_runtime_handle}
        ),
    )
    ...
    assert destroyed == [("runtime-1", "local", False)]
