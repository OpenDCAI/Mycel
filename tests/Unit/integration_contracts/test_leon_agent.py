...
async def test_leon_agent_default_runtime_ignores_missing_configured_file_skill_path(tmp_path):
    ...
    (tmp_path / ".leon").mkdir()
    (tmp_path / ".leon" / "runtime.json").write_text(
        json.dumps({"skills": {"enabled": True, "paths": [str(missing_skills)], "skills": {}}}),
        encoding="utf-8",
    )
...
def test_build_rules_section_unifies_core_risk_and_tool_preferences():
    ...
    assert "Don't add error handling, patch paths, or validation for scenarios that can't happen" in rules
