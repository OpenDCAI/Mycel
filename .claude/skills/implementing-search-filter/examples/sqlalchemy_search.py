...
from sqlalchemy import Boolean, Column, DateTime, Float, Index, Integer, String, Text, create_engine, func, or_
...
class ProductSearcher:
    ...
    def _add_text_search(self, query: Query, search_term: str) -> Query:
        ...
        if self.session.bind.dialect.name == "postgresql":
            ...
            search_vector = func.to_tsvector(
                "english",
                func.coalesce(Product.title, "") + " " + func.coalesce(Product.description, "") + " " + func.coalesce(Product.tags, ""),
            )
            ...
            query = query.add_columns(func.ts_rank(search_vector, search_query).label("relevance"))
        else:
            # Fallback to LIKE for other databases
            search_pattern = f"%{search_term}%"
            query = query.filter(
                or_(Product.title.ilike(search_pattern), Product.description.ilike(search_pattern), Product.tags.ilike(search_pattern))
            )
    ...
    def _get_facets(self, base_query: Query, active_filters: dict[str, Any]) -> dict[str, list[dict]]:
        ...
        category_facets = (
            category_query.with_entities(Product.category, func.count(Product.id).label("count")).group_by(Product.category).all()
        )

        facets["categories"] = [{"value": cat, "count": count} for cat, count in category_facets if cat]

        # Brand facets
        brand_query = self._get_base_facet_query(base_query, active_filters, "brands")
        brand_facets = brand_query.with_entities(Product.brand, func.count(Product.id).label("count")).group_by(Product.brand).all()

        facets["brands"] = [{"value": brand, "count": count} for brand, count in brand_facets if brand]
        ...
        facets["availability"] = [{"value": "in_stock", "count": in_stock_count}]

        return facets

    def _get_base_facet_query(self, base_query: Query, active_filters: dict[str, Any], exclude_filter: str) -> Query:
        """
    ...
    def _get_price_range_facets(self, base_query: Query, active_filters: dict[str, Any]) -> list[dict]:
        ...
        query_without_price = self._get_base_facet_query(base_query, active_filters, "price")
        ...
        for min_price, max_price, label in ranges:
            ...
            if count > 0:
                facets.append({"value": f"{min_price}-{max_price or 'inf'}", "label": label, "count": count})
...
if __name__ == "__main__":
    ...
    results = searcher.search(
        query="laptop",
        filters={"categories": ["Electronics", "Computers"], "min_price": 500, "max_price": 2000, "in_stock": True},
        sort_by="price_asc",
        page=1,
        per_page=20,
        include_facets=True,
