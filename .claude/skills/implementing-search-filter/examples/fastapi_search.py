...
class SearchFilters(BaseModel):
    ...
    categories: list[str] | None = Field(None, max_items=20, description="Product categories")
    brands: list[str] | None = Field(None, max_items=20, description="Product brands")
    min_price: float | None = Field(None, ge=0, le=1000000, description="Minimum price")
    max_price: float | None = Field(None, ge=0, le=1000000, description="Maximum price")
    in_stock: bool | None = Field(None, description="Only show in-stock items")
    min_rating: float | None = Field(None, ge=1, le=5, description="Minimum rating")
...
class SearchRequest(BaseModel):
    ...
    query: str | None = Field(None, min_length=1, max_length=200, description="Search query")
...
class MockSearchService:
    ...
    async def search(self, request: SearchRequest) -> dict[str, Any]:
        ...
        facets = {
            ...
            "brands": [{"value": "BrandX", "count": 100}, {"value": "BrandY", "count": 80}, {"value": "BrandZ", "count": 45}],
            "price_ranges": [
        ...
        return {"products": products, "total": 175, "facets": facets if request.include_facets else None}


# API Endpoints
@app.get("/api/v1/search", response_model=SearchResponse, summary="Search products (GET)")
async def search_get(
    q: str | None = Query(None, min_length=1, max_length=200, description="Search query"),
    category: list[str] | None = Query(None, description="Filter by categories"),
    brand: list[str] | None = Query(None, description="Filter by brands"),
    min_price: float | None = Query(None, ge=0, description="Minimum price"),
    max_price: float | None = Query(None, ge=0, description="Maximum price"),
    in_stock: bool | None = Query(None, description="Only in-stock items"),
    ...
    search_request = SearchRequest(
        query=q,
        filters=SearchFilters(categories=category, brands=brand, min_price=min_price, max_price=max_price, in_stock=in_stock),
        sort_by=sort,
        page=page,
        per_page=per_page,
    ...
    if cached_result:
        ...
        return SearchResponse(**cached_result, query_time_ms=query_time, cached=True)
    ...
    query_time = (time.time() - start_time) * 1000
    return SearchResponse(**response_data, query_time_ms=query_time, cached=False)


@app.post("/api/v1/search", response_model=SearchResponse, summary="Search products (POST)")
async def search_post(request: SearchRequest, service: MockSearchService = Depends(get_search_service)):
    """
...
async def health_check():
    ...
    """
    return {"status": "healthy", "timestamp": datetime.utcnow(), "cache_size": len(search_cache.cache), "version": "1.0.0"}
...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    return JSONResponse(
        status_code=400, content={"error": "Invalid input", "message": str(exc), "timestamp": datetime.utcnow().isoformat()}
    )
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        status_code=exc.status_code, content={"error": "Request failed", "message": exc.detail, "timestamp": datetime.utcnow().isoformat()}
    )
