...
class ProductFilter(df.FilterSet):
    ...
    category = df.ModelMultipleChoiceFilter(field_name="category__slug", to_field_name="slug", queryset=Category.objects.all())
    brand = df.ModelMultipleChoiceFilter(field_name="brand__slug", to_field_name="slug", queryset=Brand.objects.all())
    ...
    tags = df.ModelMultipleChoiceFilter(field_name="tags__name", to_field_name="name", queryset=Tag.objects.all())
    ...
    def search(self, queryset, name, value):
        ...
        if connection.vendor == "postgresql":
            ...
            return (
                ...
                .filter(Q(title__icontains=value) | Q(description__icontains=value))
                .order_by("-rank")
            )
    ...
    class Meta:
        model = Product
        fields = ["q", "category", "brand", "min_price", "max_price", "in_stock", "min_rating", "tags"]
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ["id", "title", "description", "category", "brand", "price", "rating", "in_stock", "tags", "created_at"]
...
class ProductViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    filter_backends = [FacetedSearchBackend, df.DjangoFilterBackend, filters.OrderingFilter]
    ...
    def get_queryset(self):
        ...
        queryset = queryset.annotate(review_count=Count("reviews", distinct=True), avg_rating=Avg("reviews__rating"))
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response({"products": serializer.data, "facets": facets})
    ...
    def _get_facets(self, queryset, request):
        ...
        # Category facets
        category_facets = queryset.values("category__name", "category__slug").annotate(count=Count("id")).order_by("-count")[:20]

        facets["categories"] = [
            {"value": item["category__slug"], "label": item["category__name"], "count": item["count"]} for item in category_facets
        ]

        # Brand facets
        brand_facets = queryset.values("brand__name", "brand__slug").annotate(count=Count("id")).order_by("-count")[:20]

        facets["brands"] = [{"value": item["brand__slug"], "label": item["brand__name"], "count": item["count"]} for item in brand_facets]
        ...
        for min_price, max_price, label in price_ranges:
            ...
            if count > 0:
                price_facets.append({"value": f"{min_price}-{max_price or 'inf'}", "label": label, "count": count})
    ...
    def autocomplete(self, request):
        ...
        suggestions = (
            Product.objects.filter(Q(title__icontains=query) | Q(brand__name__icontains=query) | Q(category__name__icontains=query))
            .values("title")
            .distinct()[:10]
        )

        # Format response
        return Response({"query": query, "suggestions": [{"text": item["title"], "type": "product"} for item in suggestions]})
    ...
    def export(self, request):
        ...
        for product in queryset:
            writer.writerow([product.id, product.title, product.category.name, product.brand.name, product.price, product.in_stock])
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(self.style.SUCCESS("Successfully created search index"))
