...
class SearchParamValidator:
    ...
    RULES = {
        ...
        "brands": {"type": list, "max_items": 20, "item_type": str},
        "min_price": {"type": (int, float), "min_value": 0, "max_value": 1000000},
        "max_price": {"type": (int, float), "min_value": 0, "max_value": 1000000},
        "sort_by": {"type": str, "allowed_values": ["relevance", "price_asc", "price_desc", "newest", "oldest", "rating", "popularity"]},
        "page": {"type": int, "min_value": 1, "max_value": 100},
        "per_page": {"type": int, "min_value": 1, "max_value": 100, "default": 20},
        "in_stock": {"type": bool},
        "date_from": {"type": str, "date_format": "%Y-%m-%d"},
        "date_to": {"type": str, "date_format": "%Y-%m-%d"},
    }
    ...
    def __init__(self, allowed_categories: list[str] | None = None):
    ...
    def _validate_param(self, name: str, value: Any, rule: dict) -> dict:
        ...
        if expected_type and not isinstance(value, expected_type):
            result["valid"] = False
            result["errors"].append(f"{name}: Expected {expected_type.__name__}, got {type(value).__name__}")
            return result
    ...
    def _validate_string(self, name: str, value: str, rule: dict) -> dict:
        ...
        for pattern in self.SQL_INJECTION_PATTERNS:
            if re.search(pattern, value, re.IGNORECASE):
                result["valid"] = False
                result["errors"].append(f"{name}: Potential SQL injection detected")
                return result
        ...
        if len(value) < min_len:
            result["valid"] = False
            result["errors"].append(f"{name}: Must be at least {min_len} characters")

        if len(value) > max_len:
            result["valid"] = False
            result["errors"].append(f"{name}: Must be at most {max_len} characters")

        # Pattern validation
        pattern = rule.get("pattern")
        if pattern and not re.match(pattern, value):
            result["valid"] = False
            result["errors"].append(f"{name}: Contains invalid characters")

        # Allowed values validation
        allowed = rule.get("allowed_values")
        if allowed and value not in allowed:
            result["valid"] = False
            result["errors"].append(f"{name}: Must be one of {allowed}")
    ...
    def _validate_list(self, name: str, value: list, rule: dict) -> dict:
        ...
        if len(value) > max_items:
            result["valid"] = False
            result["errors"].append(f"{name}: Cannot have more than {max_items} items")
        ...
        if item_type:
            for i, item in enumerate(value):
                if not isinstance(item, item_type):
                    result["valid"] = False
                    result["errors"].append(f"{name}[{i}]: Expected {item_type.__name__}")
        ...
        if allowed:
            ...
            if invalid_items:
                result["valid"] = False
                result["errors"].append(f"{name}: Invalid items: {invalid_items}")
    ...
    def _validate_number(self, name: str, value: float, rule: dict) -> dict:
        ...
        if value < min_val:
            result["valid"] = False
            result["errors"].append(f"{name}: Must be at least {min_val}")

        if value > max_val:
            result["valid"] = False
            result["errors"].append(f"{name}: Must be at most {max_val}")
    ...
    def _validate_date(self, name: str, value: str, date_format: str) -> dict:
        ...
        try:
            ...
            if parsed_date.date() > date.today():
                result["errors"].append(f"{name}: Date cannot be in the future")
        except ValueError:
            result["valid"] = False
            result["errors"].append(f"{name}: Invalid date format (expected {date_format})")
    ...
    def _apply_defaults(self, params: dict) -> dict:
        ...
        defaults = {"page": 1, "per_page": 20, "sort_by": "relevance"}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate search parameters")

    parser.add_argument("--params", type=str, required=True, help="JSON string of search parameters")
    ...
    # Output results
    result = {"valid": is_valid, "cleaned_params": cleaned_params, "errors": errors}
