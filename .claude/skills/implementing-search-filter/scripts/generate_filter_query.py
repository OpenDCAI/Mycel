...
class SQLQueryBuilder:
    ...
    def build_search_query(self, filters: dict[str, Any]) -> str:
        ...
        if filters.get("min_price") or filters.get("max_price"):
            self._add_price_filter(filters.get("min_price"), filters.get("max_price"))
        ...
        if filters.get("date_from") or filters.get("date_to"):
            self._add_date_filter(filters.get("date_from"), filters.get("date_to"))

        # Sorting
        self._add_sorting(filters.get("sort_by", "relevance"))

        # Pagination
        self._add_pagination(filters.get("page", 1), filters.get("per_page", 20))
        ...
        if self.dialect == "postgresql":
            ...
            search_vector = """
                to_tsvector('english', COALESCE(p.title, '') || ' ' ||
                                       ...
                                       COALESCE(p.tags, ''))
            """
            self.query_parts["where"].append(f"{search_vector} @@ plainto_tsquery('english', '{query}')")

            # Add relevance score
            self.query_parts["select"].append(f"ts_rank({search_vector}, plainto_tsquery('english', '{query}')) AS relevance")
        else:
            # MySQL FULLTEXT
            self.query_parts["where"].append(f"MATCH(p.title, p.description) AGAINST('{query}' IN NATURAL LANGUAGE MODE)")
    ...
    def _add_price_filter(self, min_price: float | None, max_price: float | None):
    ...
    def _add_date_filter(self, date_from: str | None, date_to: str | None):
...
class ElasticsearchQueryBuilder:
    ...
    def build_search_query(self, filters: dict[str, Any]) -> dict:
        ...
        # Category filter
        if filters.get("categories"):
            query["query"]["bool"]["filter"].append({"terms": {"category.keyword": filters["categories"]}})
        ...
        # Brand filter
        if filters.get("brands"):
            query["query"]["bool"]["filter"].append({"terms": {"brand.keyword": filters["brands"]}})
        ...
        if filters.get("date_from") or filters.get("date_to"):
            ...
            query["query"]["bool"]["filter"].append({"range": {"created_at": date_range}})
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate search queries from filter parameters")

    parser.add_argument("--type", choices=["sql", "elasticsearch"], default="sql", help="Query type to generate")

    parser.add_argument("--dialect", choices=["postgresql", "mysql"], default="postgresql", help="SQL dialect (for SQL queries)")

    parser.add_argument("--filters", type=str, required=True, help="JSON string of filter parameters")
