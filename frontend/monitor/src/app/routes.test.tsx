...
describe("MonitorRoutes", () => {
  ...
  it("uses provider session rows as the summary running truth when summary count lags", async () => {
    ...
    expect(await screen.findByText("3 运行会话")).toBeInTheDocument();
  ...
  it("surfaces provider-card live telemetry gap when only part of the running sandboxes report usage", async () => {
    mockRoutePayloads({
      "/resources": {
        summary: {
          snapshot_at: "2026-04-08T00:00:00Z",
  ...
  it("surfaces missing live telemetry truth directly on the sandbox card", async () => {
    mockRoutePayloads({
      "/resources": {
        summary: {
          snapshot_at: "2026-04-08T00:00:00Z",
  ...
  it("surfaces live usage coverage in the provider detail overview", async () => {
    mockRoutePayloads({
      "/resources": {
        summary: {
          snapshot_at: "2026-04-08T00:00:00Z",
  ...
  it("surfaces live telemetry gap in the local provider detail overview", async () => {
    mockRoutePayloads({
      "/resources": {
        summary: {
          snapshot_at: "2026-04-08T00:00:00Z",
    ...
    expect(detailLabel).toBeDefined();
    if (!detailLabel) {
      throw new Error("Expected local provider detail telemetry-gap metric");
    }
    expect(detailLabel.nextElementSibling).toHaveTextContent("2");
  });
