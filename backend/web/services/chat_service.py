...
class ChatService:
    ...
    def send_message(
        ...
        logger.debug("[send_message] chat=%s sender=%s content=%.50s signal=%s", chat_id[:8], sender_id[:15], content[:50], signal)
        mentions = mentioned_ids or []
        now = time.time()
        msg_id = str(uuid.uuid4())
        msg = ChatMessageRow(
            id=msg_id, chat_id=chat_id, sender_id=sender_id,
            content=content, mentioned_ids=mentions, created_at=now,
        )
        ...
        for ce in participants:
            ...
            if self._delivery_resolver:
                ...
                action = self._delivery_resolver.resolve(
                    ce.entity_id, chat_id, sender_id, is_mentioned=is_mentioned,
                )
                if action != DeliveryAction.DELIVER:
                    logger.info("[deliver] POLICY %s for %s (sender=%s chat=%s mentioned=%s)", action.value, ce.entity_id, sender_id, chat_id[:8], is_mentioned)
                    continue
