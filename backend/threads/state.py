...
def _runtime_row_from_binding(runtime_repo: Any, binding: Any) -> dict[str, Any] | None:
    ...
    runtime_row = find_by_instance(provider_name=binding.provider_name, instance_id=provider_env_id)
    if runtime_row is None:
        return None
    if not isinstance(runtime_row, dict):
        raise RuntimeError("runtime repository find_by_instance must return an object row")
    return runtime_row
