...
| 操作 | 接口 |
|------|------|
| 列出运行时 | `GET /api/sandbox/runtimes` |
| 暂停 | `POST /api/sandbox/runtimes/{id}/pause?provider={type}` |
| 恢复 | `POST /api/sandbox/runtimes/{id}/resume?provider={type}` |
| 销毁 | `DELETE /api/sandbox/runtimes/{id}?provider={type}` |
| 获取指标 | `GET /api/sandbox/runtimes/{id}/metrics` |
...
启用 SQLite 沙箱存储时，使用 `LEON_SANDBOX_DB_PATH` 指定沙箱运行态数据库：

| 表 | 用途 |
|----|------|
| `sandbox_leases` | 内部沙箱生命周期桥 — 提供商、期望状态/实际状态 |
| `sandbox_instances` | 提供商侧的原生运行时 ID |
